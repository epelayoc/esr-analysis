...
def _load_env_secrets() -> None:
    """Verifica la presencia de variables de entorno."""
    if "HF_TOKEN" not in os.environ or "AESGCM_KEY" not in os.environ:
        logger.warning("No se encontraron HF_TOKEN o AESGCM_KEY en las variables de entorno.")
...
# ---------------------------------------------------------------------------
# Carga automática del HF_TOKEN (Google Colab, Kaggle o Sistema Local)
# ---------------------------------------------------------------------------
def _load_hf_token() -> None:
    """Intenta cargar el HF_TOKEN desde Google Colab Secrets, Kaggle Secrets,
    o del entorno local de la máquina."""
    hf_token = os.environ.get("HF_TOKEN")

    # 1. Intentar desde Google Colab Secrets
    if not hf_token and IN_COLAB and colab_userdata is not None:
        try:
            hf_token = colab_userdata.get("HF_TOKEN")
        except Exception:
            pass

    # 2. Intentar desde Kaggle Secrets (por si estás ejecutando en Kaggle)
    if not hf_token:
        try:
    ...
    # 3. Si se encontró en alguna de las fuentes, inyectarlo en el sistema
    if hf_token:
        os.environ["HF_TOKEN"] = hf_token
        # Opcional: Autenticar también usando huggingface_hub si está instalado
        try:
        ...
        logger.warning(
            "⚠️ No se encontró HF_TOKEN en os.environ, Colab Secrets ni Kaggle Secrets. "
            "Modelos restringidos o privados podrían fallar al descargarse."
        )
...
class ESRExtractor:
    ...
    def extract(self, file_path: str) -> ESR:
        ...
        txt = re.sub(self.FOOTER_PATTERN, "", txt, flags=re.DOTALL) # strip_footer
...
class ESRAnonymizer:
    ...
    def _clean_text(self, text: Any, number: Any, acronym: Any, title: Any) -> str:
        ...
        text = re.sub( self.CONTEXT_PATTERN, lambda m: m.group(0) if m.group(0) in self.whitelist else "[NAME]", text)
        text = re.sub(self.DOUBLE_WORD_PATTERN, "[NAME]", text)   
        text = re.sub( self.ACRONYM_PATTERN, 
            lambda m: ( m.group(0) if (m.group(0) not in self.entities and m.group(0) in self.blacklist) else "[NAME]"),
            text,
        )
        return text
...
class ESRExporter:
    ...
    # -- lectura/escritura de libros con varias hojas -----------------------
    # Usadas tanto por la etapa de clasificación (una hoja por bloque +
    # resúmenes) como por la etapa de resumen LLM (una hoja por bloque con
    # el resumen añadido), para no duplicar la lógica de ``pd.ExcelWriter``
    # / ``pd.read_excel(sheet_name=None)`` en cada etapa.
    @staticmethod
    def read_excel_sheets(
        input_path: Union[str, Path],
        sheet_name: Optional[Union[str, List[str]]] = None,
    ) -> Dict[str, pd.DataFrame]:
        """Lee un Excel devolviendo un dict {nombre_hoja: DataFrame}.

        Si ``sheet_name`` es ``None`` se devuelven todas las hojas del
        libro; si es un str o una lista, solo esas hojas (pandas ya
        devuelve un dict en ambos casos gracias a ``sheet_name=None`` o a
        pasar una lista).
        """
        result = pd.read_excel(
            input_path,
            sheet_name=list(sheet_name) if isinstance(sheet_name, (list, tuple)) else sheet_name,
        )
        if isinstance(result, pd.DataFrame):
            # pandas devuelve un único DataFrame (no un dict) cuando
            # sheet_name es un str suelto; lo normalizamos a dict.
            name = sheet_name if isinstance(sheet_name, str) else 0
            return {name: result}
        return result

    @staticmethod
    def to_excel_multi(sheets: Dict[str, pd.DataFrame], output_path: Union[str, Path]) -> str:
        """Escribe varios DataFrames como hojas de un mismo libro Excel.

        Las hojas vacías se omiten (mismo criterio que se usaba antes en
        ...
        logger.info(f"📄 Excel (múltiples hojas) guardado correctamente en: {output_path}")
        return output_path
...
class ESRReportGenerator:
    ...
    def build_workbook_sheets(
        ...
        """Construye TODAS las hojas del libro de clasificación a partir del
        DataFrame de resultados combinados (``All_raw_data`` + una hoja por
        bloque + un resumen por bloque agrupado por Activity).

        Esta es la única responsable de "agrupar" (pivotar / agregar por
        Activity); antes esta lógica vivía mezclada dentro de
        ``ESRClassificationIO.save_results``, que debería limitarse a
        persistir. Devuelve un dict {nombre_hoja: DataFrame} listo para que
        una capa de IO (``ESRExporter.to_excel_multi`` /
        ``ESRClassificationIO.save_results``) lo escriba a disco, sin que la
        IO tenga que saber nada sobre cómo se construyen esas hojas.
        """
        sheets: dict[str, pd.DataFrame] = {}

        all_raw_df = cls.build_all_raw_data_sheet(df)
        if not all_raw_df.empty:
            sheets["All_raw_data"] = all_raw_df

        block_sheets = cls.build_block_sheets(df)
        for block_name, sheet_df in block_sheets.items():
            if not sheet_df.empty:
                sheets[BLOCK_SHEET_NAMES.get(block_name, block_name)] = sheet_df

        summaries = cls.extract_top_sentences_and_group(
        ...
        for block_name, summary_df in summaries.items():
            if not summary_df.empty:
                sheets[BLOCK_SUMMARY_SHEET_NAMES.get(block_name, block_name)] = summary_df
...
class ESRClassificationPipeline:
    def __init__(self, topic_index: TopicIndex, io: Optional["ESRClassificationIO"] = None):
        _ensure_nltk_punkt()
        self.topic_index = topic_index
        self.analyzer = ESRAnalyzer(topic_index)
        # Nota: ``ESRClassificationIO`` se define más abajo en el módulo
        # (hereda de ``ESRExporter``); se referencia aquí solo como texto
        # ("ESRClassificationIO") en el type hint para evitar problemas de
        # orden de definición, y se resuelve de forma perezosa por defecto.
        self.io = io or ESRClassificationIO()
    ...
    def analysis(
        ...
        output_path = self.io.resolve_output_path(output_path, output_format, num_rows_to_process)
        self.io.save_results(
            final_combined_df,
            output_path,
            output_format,
...
class ESRClassificationIO(ESRExporter):
    """IO de la etapa de clasificación: lee el Excel de entrada (extracción)
    y persiste el libro de resultados clasificados.

    Hereda de ``ESRExporter`` en lugar de reimplementar la lectura/escritura
    de Excel/CSV/Parquet: así se reutilizan ``to_excel`` / ``to_parquet`` /
    ``read_excel`` / ``to_excel_multi`` y, de paso, la capacidad de
    anonimizar/desanonimizar (``anonymize`` / ``deanonymize`` /
    ``excel_encode`` / ``excel_decode``) que ya tenía ``ESRExporter`` para la
    etapa de extracción. A diferencia de la versión anterior, esta clase ya
    NO agrupa ni pivota nada: esa responsabilidad es de
    ``ESRReportGenerator.build_workbook_sheets``; aquí solo se persiste lo
    que esa función ya ha construido.
    """

    def __init__(self, anonymizer: Optional[ESRAnonymizer] = None) -> None:
        super().__init__(anonymizer=anonymizer)
    ...
    def save_results(
        ...
        """Persiste los resultados de clasificación. El agrupamiento por
        Activity/bloque (``All_raw_data``, hojas por bloque, resúmenes) se
        delega íntegramente en ``ESRReportGenerator.build_workbook_sheets``;
        esta clase únicamente escribe a disco (Parquet y/o Excel)."""
        ...
        if output_format in ("parquet", "both"):
            ...
            self.to_parquet(df, parquet_path)

        if output_format in ("excel", "both"):
            excel_path = output_path.with_suffix(".xlsx")
            sheets = ESRReportGenerator.build_workbook_sheets(df, top_n_sentences=top_n_sentences)
            try:
                self.to_excel_multi(sheets, excel_path)
            except Exception as e:
                logger.error("Failed to write Excel file: %s", e)
                raise

    def load_input_dataframe(self, input_path: str | Path) -> pd.DataFrame:
        """Carga el Excel de entrada para clasificar (típicamente la salida
        de la etapa de extracción). Reutiliza ``ESRExporter.read_excel``; si
        el fichero fue previamente cifrado con ``ESRAnonymizer`` y esta
        instancia tiene un ``anonymizer`` con cifra configurada, se puede
        desanonimizar a posteriori llamando a ``self.deanonymize(df)``."""
        input_path = Path(input_path)
        if input_path.exists():
            return self.read_excel(input_path)
        if IN_COLAB:
            uploaded = colab_files.upload()
...
class ExcelSummaryProcessor:
    ...
    def __init__(
        self,
        llm_engine: LLMSummarizerEngine,
        batch_size: int = 4,
        io: Optional[ESRExporter] = None,
    ):
        self.llm_engine = llm_engine
        self.batch_size = batch_size
        # Reutiliza la misma capa de IO (Excel/CSV/Parquet, + anonimización
        # opcional) que las etapas de extracción y clasificación, en lugar
        # de leer/escribir el Excel "a mano" con pandas directamente.
        self.io = io or ESRExporter()
    ...
    def process_excel(
        ...
        all_sheets = self.io.read_excel_sheets(input_excel_path)
        ...
        # A diferencia de ``ESRClassificationIO.save_results``, aquí se
        # escriben también las hojas vacías/sin procesar (se copian tal
        # cual), así que se usa directamente ``pd.ExcelWriter`` en vez de
        # ``self.io.to_excel_multi`` (que omite hojas vacías). Se mantiene
        # sobre el mismo motor ("openpyxl") por coherencia con el resto de
        # la capa de IO.
...
class ESRMasterPipeline:
    ...
    def __init__(
        ...
        io: Optional["ESRClassificationIO"] = None,
    ) -> None:
        self.extraction_pipeline = extraction_pipeline
        self.topic_index = topic_index
        # Clave (model_name, device) con la que se construyó `self.topic_index`,
        # para poder detectar si hay que reconstruirlo (p.ej. si se pide un
        # modelo distinto) en lugar de reutilizar uno "compatible por
        # casualidad" o, a la inversa, recargar el modelo sin necesidad.
        self._topic_index_key: Optional[Tuple[str, Optional[str]]] = None
        self.llm_engine = llm_engine

        # Única instancia de IO compartida por las etapas de clasificación
        # y resumen (ambas heredan/usan ``ESRExporter``), reutilizando así
        # el mismo lector/escritor de Excel y, si se configura un
        # ``ESRAnonymizer``, la misma capacidad de anonimizar/desanonimizar
        # que usa la etapa de extracción. Se expone como propiedad
        # (``self.io``) en lugar de un atributo fijado en __init__ porque
        # ``extraction_pipeline`` puede asignarse *después* de construir el
        # ``ESRMasterPipeline`` (así lo hace la CLI); de esta forma, si no se
        # pasó explícitamente un ``io``, se construye de forma perezosa
        # reutilizando el anonymizer de la extracción en el momento en que
        # realmente se necesita (clasificación/resumen), no en __init__.
        self._io = io

    @property
    def io(self) -> "ESRClassificationIO":
        if self._io is None:
            self._io = ESRClassificationIO(
                anonymizer=self.extraction_pipeline.anonymizer if self.extraction_pipeline else None
            )
        return self._io

    @io.setter
    def io(self, value: "ESRClassificationIO") -> None:
        self._io = value
    ...
    def _get_topic_index(
        ...
        """Construye (o reutiliza) el ``TopicIndex`` con el modelo de
        embeddings, garantizando que el modelo se carga una única vez por
        combinación (model_name, device) durante la vida de este
        ``ESRMasterPipeline``.

        Antes se cacheaba solo con "``if self.topic_index is None``", lo que
        por un lado podía ocultar un cambio de modelo entre llamadas (se
        seguía usando el antiguo en silencio) y por otro no dejaba claro que
        el modelo NUNCA se recarga innecesariamente durante una misma
        ejecución (extracción -> clasificación -> resumen usan la misma
        instancia). Con la clave (model_name, device) se cubren ambos casos.
        """
        cache_key = (model_name, device)
        if self.topic_index is None or self._topic_index_key != cache_key:
            logger.info(f"Cargando modelo de embeddings '{model_name}' (device={device or 'auto'})...")
            self.topic_index = TopicIndex.build(model_name=model_name, device=device)
            self._topic_index_key = cache_key
    ...
    def run_classification(
        ...
        if df is None:
            ...
            df = self.io.load_input_dataframe(input_path)

        topic_index = self._get_topic_index(model_name=model_name, device=device)
        pipeline = ESRClassificationPipeline(topic_index, io=self.io)
    ...
    def run_summary(
        ...
        processor = ExcelSummaryProcessor(llm_engine=engine, batch_size=batch_size, io=self.io)
